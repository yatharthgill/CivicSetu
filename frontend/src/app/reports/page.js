...
export default function ReportsPage() {
    ...
    const sameCityOnly = searchParams.get('sameCity') === 'true';
    ...
    const { data: reportsData, isLoading, error } = useQuery({
        queryKey: ['reports', filter, location, detectedCity, sameCityOnly],
        queryFn: async () => {
            if (filter === 'all') {
        ...
        enabled: !!user && (
            filter === 'all' ||
            filter === 'my' ||
            (filter === 'nearby' && !!location && (!sameCityOnly || !!detectedCity) && !isResolvingCity)
        ),
    });
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            {isLoading || isPreparingNearbyResults ? (
