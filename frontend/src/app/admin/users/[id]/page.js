...
import { Loader2, ArrowLeft, Mail, Calendar, Shield, AlertTriangle, MapPin } from 'lucide-react';
                ...
                <div className="lg:col-span-1 space-y-6">
                    <div className="bg-white shadow-sm rounded-lg border border-gray-200 p-4 sm:p-6">
                        <div className="flex flex-col items-center">
                            <div className="h-20 w-20 sm:h-24 sm:w-24 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 text-2xl sm:text-3xl font-bold mb-4">
                                {user.name.charAt(0).toUpperCase()}
                            </div>
                            <h2 className="text-xl sm:text-2xl font-bold text-gray-900 text-center">{user.name}</h2>
                        ...
                        <div className="mt-6 sm:mt-8 border-t border-gray-100 pt-6">
                            ...
                            <div className="space-y-3">
                                <button
                                    ...
                                    className={`w-full py-2.5 px-4 rounded-md text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 min-h-[44px] ${user.isActive
                                ...
                                {user.role === 'user' ? (
                                    <button
                                        ...
                                        className="w-full py-2.5 px-4 rounded-md text-sm font-medium bg-purple-50 text-purple-700 hover:bg-purple-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 min-h-[44px]"
                                    >
                                        Promote to Admin
                                    </button>
                                ) : (
                                    <button
                                        ...
                                        className="w-full py-2.5 px-4 rounded-md text-sm font-medium bg-gray-50 text-gray-700 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 min-h-[44px]"
                                    >
                                        Demote to User
                                    </button>
                                )}
                    ...
                    <div className="bg-white shadow-sm rounded-lg border border-gray-200 p-4 sm:p-6">
                        ...
                        <div className="grid grid-cols-2 gap-3 sm:gap-4">
                            <div className="bg-gray-50 p-3 sm:p-4 rounded-lg text-center">
                                <div className="text-xl sm:text-2xl font-bold text-gray-900">{stats?.totalReports || 0}</div>
                                <div className="text-xs text-gray-500 uppercase mt-1">Total Complaints</div>
                            </div>
                            <div className="bg-green-50 p-3 sm:p-4 rounded-lg text-center">
                                <div className="text-xl sm:text-2xl font-bold text-green-700">{stats?.resolved || 0}</div>
                            ...
                            <div className="bg-red-50 p-3 sm:p-4 rounded-lg text-center">
                                <div className="text-xl sm:text-2xl font-bold text-red-700">{stats?.rejected || 0}</div>
                                <div className="text-xs text-red-600 uppercase mt-1">Rejected</div>
                            </div>
                            <div className="bg-blue-50 p-3 sm:p-4 rounded-lg text-center">
                                <div className="text-xl sm:text-2xl font-bold text-blue-700">{stats?.totalUpvotes || 0}</div>
                            ...
                            <div className="overflow-x-auto">
                                <table className="min-w-full divide-y divide-gray-200">
                                    <thead className="bg-gray-50">
                                        <tr>
                                            <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Complaint</th>
                                                    ...
                                                    <div className="flex items-start">
                                                        <div className="flex-shrink-0 h-10 w-10 bg-gray-200 rounded-md overflow-hidden">
                                                            {report.media && report.media.length > 0 && report.media[0].type === 'image' ? (
                                                                <img src={report.media[0].thumbnail || report.media[0].url} alt="" className="h-full w-full object-cover" loading="lazy" />
                                                            ) : (
